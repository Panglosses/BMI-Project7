...
# 使用示例:
# python solvent_accessibility.py --wet-pdb pdb/SUMO1_water.pdb --dry-pdb pdb/SUMO1.pdb --method peratom --threshold 3.5 --margin 2.0 --fraction-threshold 0.3 --min-hits 2 --small-residue-size 5 --verbose
...
def count_waters_within_R(res_coords, water_coords=None, R=5.0, chunk=5000, tree=None):
    ...
    if tree is not None:
        return np.array(
            [len(tree.query_ball_point(rc, R)) for rc in res_coords], dtype=int
        )
...
def collect_peratom_dists(struct, residues, water_coords=None, tree=None):
    ...
    if tree is None and water_coords is not None and water_coords.size != 0:
        tree = KDTree(water_coords)
    ...
    for r in residues:
        ...
        if tree is not None:
            ...
            if water_coords is None or getattr(water_coords, "size", 0) == 0:
...
def peratom_flags_from_dists(
    ...
    for r in residues:
        ...
        is_small = (r.resname.upper() in small_res_set) or (
            n_atoms <= small_residue_size
        )
...
def run_custom_accessibility(
    ...
    if verbose:
        print(
            f"Parsed {len(residues)} residues and {water_coords.shape[0]} water oxygens"
        )

    res_coords = (
        np.vstack([r.coord for r in residues])
        if len(residues) > 0
        else np.zeros((0, 3))
    )
    min_d_centroid = compute_min_distances_chunked(
        res_coords, water_coords, chunk=chunk
    )

    water_tree = KDTree(water_coords) if water_coords.size != 0 else None

    counts = count_waters_within_R(
        res_coords, water_coords=water_coords, R=R, chunk=chunk, tree=water_tree
    )
    ...
    dists_map, n_atoms_map, min_d_map = collect_peratom_dists(
        struct, residues, water_coords=water_coords, tree=water_tree
    )
        ...
        min_d_final = np.array(
            [
                (
                    ...
                    if (r.chain, str(r.resnum)) in min_d_map
                    else min_d_centroid[i]
                )
    ...
    for idx, (r, d_key) in enumerate(zip(residues, min_d_final)):
        ...
        rows_peratom.append(
            [r.chain, r.resnum, r.resname, f"{float(d_key):.3f}", nwater, flag]
        )
    write_csv(
        out_peratom,
        [
...
from compute_residue_sasa import compute_residue_sasa
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Combined solvent-accessibility + FreeSASA comparison"
    )
    parser.add_argument(
        "--wet-pdb", required=True, help="solvated pdb (for custom method)"
    )
    parser.add_argument(
        "--dry-pdb", required=True, help="unsolvated pdb (for FreeSASA)"
    )
    ...
    # ---------------------------------------------------------
    sasa_results = compute_residue_sasa(args.dry_pdb, WATER_NAMES, ACCESS_THRESHOLD)
    out_freesasa = args.dry_pdb.replace(".pdb", "_freesasa.csv")
    write_csv(
        out_freesasa, ["chain", "resnum", "resname", "SASA", "Accessible"], sasa_results
    )
    ...
    sasa_map = {
        (normalize_chain(c), str(r)): acc for c, r, rn, sasa, acc in sasa_results
    }
    custom_map = {
        (normalize_chain(r.chain), str(r.resnum)): custom_flags[i]
        for i, r in enumerate(residues)
    }
    ...
    for r in residues:
        ...
        comparison.append([r.chain, r.resnum, r.resname, c_acc, s_acc, match])
